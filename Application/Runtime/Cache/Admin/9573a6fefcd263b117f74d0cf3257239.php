<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
	<link rel="stylesheet" href="/wei/Application/Admin/Common/style/css/ch-ui.admin.css">
	<link rel="stylesheet" href="/wei/Application/Admin/Common/style/font/css/font-awesome.min.css">
</head>
<body style="background:#F3F3F4;">
	<div class="login_box">
		<h1>Manager</h1>
		<h2>欢迎使用管理平台</h2>
		<div class="form">
			<p style="color:red"><?php echo ($msg); ?></p>
			<form action="<?php echo U('Manager/login');?>" method="post">
				<ul>
					<li>
					<input type="text" name="admin_user" class="text"/>
						<span><i class="fa fa-user"></i></span>
					</li>
					<li>
						<input type="password" name="admin_psd" class="text"/>
						<span><i class="fa fa-lock"></i></span>
					</li>
					<li>
						<input type="text" class="code" name="captcha"/>
						<span><i class="fa fa-check-square-o"></i></span>
						<img src="/wei/index.php/Admin/Manager/verifyImg" alt="" onclick="this.src='/wei/index.php/Admin/Manager/verifyImg/'+Math.random()">
					</li>
					<li>
						<input type="submit" value="立即登陆"/>
					</li>
				</ul>
			</form>
			<p><a href="#">返回首页</a> &copy; 2016 Powered by <a href="http://www.houdunwang.com" target="_blank">http://www.houdunwang.com</a></p>
		</div>
	</div>
</body>
</html>